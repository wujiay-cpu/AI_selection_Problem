...
# Temporarily disable ultra C++ DFS path for stability.
# The extension can still be imported, but runtime routing avoids crash-prone calls.
_CPP_ULTRA_ENABLED = False
...
def run_algorithm(m, n, k, j, s, min_cover, selected_numbers, optimization_level=2, progress_callback=None):
    ...
    if required_cover == 1 and _CPP_AVAILABLE and _CPP_ULTRA_ENABLED:
        ...
        print(f"Active C++ Ultra Exact Solver")
        try:
            # 报告初始进度
            if progress_callback:
                # 简单用 Greedy 跑一次做进度显示
            ...
            bt_results, aborted = _cover_core.beam_search_selection(
                list(selected_numbers), k, j, s, beam_width, expand_k
            )
